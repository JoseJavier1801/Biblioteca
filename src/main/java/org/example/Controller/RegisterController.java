...
public class RegisterController {
    ...
    private void register() {
        ...
        // Validar datos
        if (!org.example.UTILS.ValidationDATA.isValidUsername(user)) {
            showmesaje("Invalid username. Must be at least 4 characters long and contain only letters and numbers.");
            return;
        }
        if (!org.example.UTILS.ValidationDATA.isValidPassword(password)) {
        ...
        if (!org.example.UTILS.ValidationDATA.isValidEmail(email)) {
            showmesaje("Invalid email address.");
            return;
        }
        if (!org.example.UTILS.ValidationDATA.isValidDNI(dni)) {
        ...
        try {
            ...
            String hashedPassword = org.example.UTILS.Encrypt.EncryptPassword(password);
